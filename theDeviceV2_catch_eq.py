...
class DeviceV2_catch_eq:
    ...
    def run(self):
        ...
        while True:  # 0 <= self.pc < len(self.program):
            inst = self.program[self.pc]
            if inst[0] == "eqrr":
                if last_reg_1 == -1:
                    print(f"The first value is {self.reg[1]}")
                print(f"\rlooking for a cycle... {len(seen)}", end="")
                if self.reg[1] in seen:
                    print("")
                    print(f"The last stopping value is {last_reg_1}")
                    break
                else:
                    seen.add(self.reg[1])
